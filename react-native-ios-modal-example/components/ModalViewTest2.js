  ...
  <ModalView ref={ref} {...props}>
    <>
      <View style={sharedStyles.titleContainer}>
        <Text style={sharedStyles.textEmoji}>{props.emoji ?? "😊"}</Text>
        <Text style={sharedStyles.textModal}>{props.title ?? "Hello #1"}</Text>
      </View>
      <View style={sharedStyles.textModalContainer}>
        <Text style={sharedStyles.textModalSubtitle}>
          {"UIBlurEffectStyle: "}
          <Text style={{ fontWeight: "bold" }}>{`${props.modalBGBlurEffectStyle}`}</Text>
        </Text>
      </View>
    </>
  </ModalView>
...
export class ModalViewTest2 extends React.PureComponent {
  ...
  _renderModals() {
    ...
    return (
      <React.Fragment>
        <TestModal ref={(r) => (this.modal1 = r)} emoji={"😊"} title={"Hello #1"} {...sharedProps} />
        <TestModal ref={(r) => (this.modal2 = r)} emoji={"😄"} title={"Hello There #2"} {...sharedProps} />
        <TestModal ref={(r) => (this.modal3 = r)} emoji={"💖"} title={"ModalView Test #3"} {...sharedProps} />
        <TestModal ref={(r) => (this.modal4 = r)} emoji={"🥺"} title={"PageSheet Modal #4"} {...sharedProps} />
        <TestModal ref={(r) => (this.modal5 = r)} emoji={"🥰"} title={"Hello World Modal #5"} {...sharedProps} />
        <TestModal ref={(r) => (this.modal6 = r)} emoji={"😙"} title={"Hello World #6"} {...sharedProps} />
        <TestModal ref={(r) => (this.modal7 = r)} emoji={"🤩"} title={"Heyyy There #7"} {...sharedProps} />
        <TestModal ref={(r) => (this.modal8 = r)} emoji={"😃"} title={"Another Modal #8"} {...sharedProps} />
        <TestModal ref={(r) => (this.modal9 = r)} emoji={"🏳️‍🌈"} title={"And Another Modal #9"} {...sharedProps} />
      </React.Fragment>
    );
    //#endregion
  }
