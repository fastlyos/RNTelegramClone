    ...
    onMove: (event, gestureState) => {
      ...
      if (isPinchGesture) {
        ...
        if (currentScale > initialScale && currentScale > nextScale) {
          ...
          const nextTranslateX = nextScale < initialScale ? initialTranslate.x : currentTranslate.x - (currentTranslate.x - initialTranslate.x) / k;
          const nextTranslateY = nextScale < initialScale ? initialTranslate.y : currentTranslate.y - (currentTranslate.y - initialTranslate.y) / k;
      ...
      if (isTapGesture && currentScale > initialScale) {
        ...
        if (nextTranslateX > leftBound) {
          nextTranslateX += -OUT_BOUND_MULTIPLIER * (nextTranslateX - leftBound);
        }
        if (nextTranslateX < rightBound) {
          nextTranslateX += -(nextTranslateX - rightBound) * OUT_BOUND_MULTIPLIER;
        }
        if (nextTranslateY > topBound) {
          nextTranslateY += -(nextTranslateY - topBound) * OUT_BOUND_MULTIPLIER;
        }
        if (nextTranslateY < bottomBound) {
          nextTranslateY += -(nextTranslateY - bottomBound) * OUT_BOUND_MULTIPLIER;
        }
